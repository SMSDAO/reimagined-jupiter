...
export class ArbitrageDatabase {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `[Database] Loaded ${this.opportunities.size} historical opportunities`,
    );
    this.loaded = true;
  }
  ...
  getStatistics(): DatabaseStats {
    ...
    const totalProfit = executed.reduce(
      (sum, o) => sum + (o.actualProfit || 0),
      0,
    );
    const averageProfit =
      executed.length > 0 ? totalProfit / executed.length : 0;
  ...
  async getHistoricalAnalysis(days: number = 7): Promise<{
    ...
    const recentOpps = Array.from(this.opportunities.values()).filter(
      (opp) => opp.timestamp >= cutoffDate,
    );
  ...
  private generateId(): string {
    ...
    const randomPart = Buffer.from(`${timestamp}-${process.hrtime.bigint()}`)
      .toString("base64")
      .slice(0, 8);
