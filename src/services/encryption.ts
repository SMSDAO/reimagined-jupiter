...
function deriveKey(password: string, salt: Buffer, iterations: number): Buffer {
  if (iterations < ENCRYPTION_CONFIG.minIterations || iterations > ENCRYPTION_CONFIG.maxIterations) {
    throw new Error(`Iterations must be between ${ENCRYPTION_CONFIG.minIterations} and ${ENCRYPTION_CONFIG.maxIterations}`);
  }
...
export function validateEncryptionInput(input: DecryptionInput): boolean {
  try {
    ...
    if (input.iterations !== undefined) {
      if (input.iterations < ENCRYPTION_CONFIG.minIterations || 
