...
export async function generateWallet(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (validateGxqSuffix(publicKeyStr)) {
      console.log(
        `✅ Generated GXQ wallet after ${i + 1} attempts: ${publicKeyStr}`,
      );
      return keypair;
    }
  }
  ...
  throw new Error(
    `Failed to generate GXQ wallet after ${maxAttempts} attempts`,
  );
}
...
export async function createUserWallet(
  ...
  if (signingMode === "SERVER_SIDE") {
    console.warn(
      "⚠️ SERVER_SIDE signing mode enabled. Private keys will be encrypted in-memory and wiped immediately.",
    );
  }
  ...
  if (signingMode === "CLIENT_SIDE") {
    console.log(
      "✅ Wallet created with CLIENT_SIDE signing (keys never leave user device)",
    );
  }
...
export function decryptWallet(
  ...
  if (wallet.signingMode === "CLIENT_SIDE") {
    console.warn(
      "⚠️ WARNING: Decrypting CLIENT_SIDE wallet server-side. Keys should remain on client device!",
    );
  }
...
export function createAuditEntry(
  ...
  return {
    ...
    fingerprintHash: options.fingerprint
      ? hashData(options.fingerprint)
      : undefined,
...
export async function validateWalletCreation(
  ...
  if (walletCount === undefined) {
    try {
      ...
      console.warn(
        "⚠️ Database query failed, using fallback validation:",
        dbError,
      );
...
export async function signWithWallet<
  T extends { sign: (signers: Keypair[]) => void },
>(
  wallet: EncryptedWallet,
  encryptionPassword: string,
  transaction: T,
