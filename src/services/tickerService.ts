...
export const PYTH_PRICE_FEEDS: Record<string, string> = {
  ...
  "PYTH/USD":
    "0bbf28e9a42a1e5f9b67c3d1c23a8c5a8d7d3b3e6e1d4f1e7e7f3f9f3f9f3f9f", // Note: Replace with actual Pyth price feed ID when available
};
...
export class TickerService extends EventEmitter {
  ...
  private async handleReconnect(): Promise<void> {
    ...
    console.log(
      `[TickerService] Reconnecting... Attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts}`,
    );
  ...
  public getStatus(): {
    ...
    const lastUpdate =
      prices.length > 0 ? Math.max(...prices.map((p) => p.publishTime)) : 0;
