...
export class FlashLoanService {
  ...
  async executeFlashLoanArbitrage(
    ...
    if (!inputMint || !outputMint) {
      console.error(
        "[FlashLoan] Invalid mints: inputMint and outputMint are required",
      );
      return null;
    }
    ...
    if (!loanAmount || loanAmount <= 0 || !Number.isFinite(loanAmount)) {
      console.error(
        "[FlashLoan] Invalid loanAmount: must be a positive finite number",
      );
      return null;
    }
    ...
    if (this.activeTransactions.has(txId)) {
      console.warn(
        "[FlashLoan] Transaction already in progress, preventing reentrancy",
      );
      return null;
    }
    ...
    try {
      ...
      console.log(
        `[FlashLoan] Dynamic priority fee: ${priorityFee} microlamports`,
      );

      // Step 2: Get real-time prices from Pyth
      const priceValidation = await this.validatePricesWithPyth(
        inputMint,
        outputMint,
      );
      if (!priceValidation.valid) {
        console.error(
          "[FlashLoan] Price validation failed:",
      ...
      const adjustedSlippageBps = await this.calculateDynamicSlippageBps(
        inputMint,
        slippageBps,
      );
      console.log(
        `[FlashLoan] Adjusted slippage: ${adjustedSlippageBps} bps (${adjustedSlippageBps / 100}%)`,
      );
      ...
      console.log(
        `[FlashLoan] Expected profit: $${isProfitable.profit.toFixed(4)}`,
      );
      ...
      if (simulation.value.err) {
        console.error(
          "[FlashLoan] Transaction simulation failed:",
  ...
  private async validatePricesWithPyth(
    ...
    try {
      ...
      if (!inputSymbol || !outputSymbol) {
        console.warn(
          "[FlashLoan] Cannot validate prices - tokens not in Pyth mapping",
        );
        return {
          valid: true,
          reason: "Price validation skipped - token not mapped",
        };
      }
      ...
      if (
        !this.pyth.isConfidenceAcceptable(
          inputPrice.price,
          inputPrice.confidence,
          1.0,
        )
      ) {
   ...
   */
  private async calculateDynamicSlippageBps(
    tokenMint: string,
  ...
  private async buildAtomicTransaction(
    ...
    try {
      ...
      console.warn(
        "[FlashLoan] Note: Flash loan borrow/repay instructions require provider-specific SDK integration (see BaseFlashLoanProvider implementations)",
      );
