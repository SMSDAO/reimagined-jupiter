...
export class RouteTemplateManager {
  ...
  async createTemplate(
    template: Omit<RouteTemplate, "id" | "createdAt">,
  ): Promise<string> {
  ...
  async recordExecution(id: string, success: boolean): Promise<void> {
    ...
    const successCount = success
      ? (template.successRate || 0) * (template.totalExecutions || 0) + 1
      : (template.successRate || 0) * (template.totalExecutions || 0);
