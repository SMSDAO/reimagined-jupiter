...
export class MEVProtection {
  ...
  private async getRecentPrioritizationFees(): Promise<{
    ...
    try {
      ...
      const percentile75 =
        feeValues[Math.floor(feeValues.length * 0.75)] || 2000;
      const percentile95 =
        feeValues[Math.floor(feeValues.length * 0.95)] || 5000;
  ...
  async calculateDynamicSlippage(
    ...
    const liquidityMultiplier = Math.max(
      0.5,
      1 / Math.sqrt(liquidityDepth / tradeSize),
    );
...
export class AutoExecutionEngine {
    ...
    if (config.profitDistribution.enabled) {
      try {
        this.profitDistribution = new ProfitDistributionService(connection, {
          ...
          reserveWalletPercentage:
            config.profitDistribution.reserveWalletPercentage,
  ...
  private async executionCycle(): Promise<void> {
    ...
    for (const preset of presets) {
      ...
      if (preset.strategy === "flash-loan" || preset.strategy === "hybrid") {
        const flashLoanOpps = await this.flashLoanArbitrage.findOpportunities(
          preset.tokens,
        );
      ...
      if (preset.strategy === "triangular" || preset.strategy === "hybrid") {
        const triangularOpps = await this.triangularArbitrage.findOpportunities(
          preset.tokens,
        );
      ...
      opportunities = opportunities.filter(
        (opp) => opp.estimatedProfit >= preset.minProfit,
      );

      if (opportunities.length > 0) {
        console.log(
          `Found ${opportunities.length} opportunities for ${preset.name}`,
        );
  ...
  private async executeOpportunity(
    ...
    if (this.executedOpportunityIds.has(opportunityId)) {
      console.log(
        "⚠️  Opportunity already executed recently, skipping duplicate...",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `  Estimated profit: $${opportunity.estimatedProfit.toFixed(4)}`,
      );
      console.log(
      ...
      if (signature) {
        console.log(
          `✅ Successfully executed arbitrage! Signature: ${signature}`,
        );
        ...
        const priorityFee =
          await this.mevProtection.calculatePriorityFee("medium");
  ...
  private async handleProfitDistribution(
    ...
    if (this.profitDistribution && config.profitDistribution.enabled) {
      try {
        ...
        if (result.success) {
          ...
          console.log(
            `   Reserve: ${(result.reserveAmount / 1e9).toFixed(4)} SOL`,
          );
          ...
          console.log(
            `   Total distributed: ${(stats.totalDistributed / 1e9).toFixed(4)} SOL (${stats.distributionCount} trades)`,
          );
        } else {
    ...
    // Fallback to old dev fee system if profit distribution is disabled
      ...
      console.log(
        `💰 Dev fee: $${devFeeAmount.toFixed(4)} (${(config.devFee.percentage * 100).toFixed(1)}%) to ${config.devFee.wallet.toBase58().slice(0, 8)}...`,
      );
  ...
  async manualExecute(opportunityId?: string): Promise<string | null> {
    ...
    if (opportunityId) {
      ...
      const opportunity =
        await this.quicknode.getCachedArbitrageOpportunity(opportunityId);
      if (opportunity) {
        await this.executeOpportunity(opportunity as ArbitrageOpportunity, {
          name: "Manual",
          minProfit: 0,
          maxSlippage: 0,
        });
    ...
    if (allOpportunities.length > 0) {
      ...
      allOpportunities.slice(0, 5).forEach((opp, i) => {
        console.log(
          `  ${i + 1}. ${opp.type} - $${opp.estimatedProfit.toFixed(4)} profit (${(opp.confidence * 100).toFixed(0)}% confidence)`,
        );
      });
