...
export class SNSDomainResolver {
  ...
  private cache: Map<string, { address: PublicKey; timestamp: number }> =
    new Map();
  private readonly CACHE_TTL_MS = 3600000; // 1 hour cache

  // SNS Program IDs
  private readonly SNS_PROGRAM_ID = new PublicKey(
    "namesLPneVptA9Z5rqUDD9tMTWEJwofgaYwp8cawRkX",
  );
  private readonly NAME_PROGRAM_ID = new PublicKey(
  ...
  async resolve(domain: string): Promise<PublicKey | null> {
    try {
      ...
      if (parts.length < 2) {
        throw new Error(
          `Invalid domain format: ${domain}. Expected format: name.tld`,
        );
      }
      ...
      if (!tldKey) {
        console.warn(
          `[SNSResolver] Unsupported TLD: ${tld}. Supported TLDs: ${Object.keys(this.TLD_REGISTRY).join(", ")}`,
        );
      ...
      if (!nameAccountKey) {
        console.error(
          `[SNSResolver] Failed to derive name account key for ${domain}`,
        );
        return null;
      }
      ...
      if (!accountInfo) {
        console.warn(
          `[SNSResolver] Domain ${domain} does not exist or is not registered`,
        );
        return null;
      }
      ...
      if (!ownerAddress) {
        console.error(
          `[SNSResolver] Failed to parse owner address from account data for ${domain}`,
        );
        return null;
      }
      ...
      console.log(
        `[SNSResolver] Successfully resolved ${domain} to ${ownerAddress.toBase58()}`,
      );
      return ownerAddress;
    } catch (error) {
  ...
  private async resolveAlternative(domain: string): Promise<PublicKey | null> {
    try {
      ...
      if (knownDomains[domain]) {
        ...
        console.log(
          `[SNSResolver] Resolved ${domain} via alternative method to ${address.toBase58()}`,
        );
        return address;
      }

      console.warn(
        `[SNSResolver] Domain ${domain} not found in alternative resolution`,
      );
      return null;
    } catch (error) {
   ...
   */
  private async getNameAccountKey(
    name: string,
    tld: string,
    ...
    try {
      ...
      const [nameAccountKey] = await PublicKey.findProgramAddress(
        [Buffer.from("name"), Buffer.from(name), Buffer.from(tld)],
        this.SNS_PROGRAM_ID,
      );
  ...
  private parseOwnerFromAccountData(data: Buffer): PublicKey | null {
    try {
      ...
      if (data.length < 64) {
        console.error(
          `[SNSResolver] Account data too short: ${data.length} bytes`,
        );
        return null;
      }
      ...
      console.error(
        `[SNSResolver] Error parsing owner from account data:`,
        error,
      );
      return null;
    }
  ...
  async resolveMultiple(
    domains: string[],
  ): Promise<Map<string, PublicKey | null>> {
  ...
  async reverseLookup(address: PublicKey): Promise<string[]> {
    try {
      console.log(
        `[SNSResolver] Performing reverse lookup for ${address.toBase58()}`,
      );
      ...
      if (domains.length > 0) {
        console.log(
          `[SNSResolver] Found cached domains: ${domains.join(", ")}`,
        );
        return domains;
      }
      ...
      console.warn(
        `[SNSResolver] Reverse lookup not fully implemented, returning empty array`,
      );
      return [];
    } catch (error) {
  ...
  registerManualMapping(domain: string, address: PublicKey): void {
    ...
    console.log(
      `[SNSResolver] Manually registered ${domain} -> ${address.toBase58()}`,
    );
  }
  ...
  static isValidDomain(domain: string): boolean {
    const parts = domain.toLowerCase().split(".");
    return (
      parts.length >= 2 && parts.every((part) => /^[a-z0-9-_]+$/.test(part))
    );
  }
}
