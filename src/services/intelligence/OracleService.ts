...
export class OracleService {
   ...
   */
  async analyzeExecution(context: AnalysisContext): Promise<OracleAnalysisResult> {
    ...
    try {
      ...
      if (activeAgents.length === 0) {
        ...
        return {
          ...
          confidence: 'LOW',
          agentResults: [],
          reasoning: 'No intelligence agents active - proceeding with default parameters',
          timestamp: new Date(),
        };
      }
    ...
    for (const agent of agents) {
      try {
        ...
        if (
          ...
          console.log(`⚠️ Risk agent aborted execution - stopping analysis early`);
          break;
        }
      } catch (error) {
        console.error(`❌ Agent ${agent.metadata.name} failed:`, error);
  ...
  private aggregateResults(results: AnalysisResult[]): OracleAnalysisResult {
    ...
    const proceedResults = results.filter((r) => r.recommendation === 'PROCEED');
