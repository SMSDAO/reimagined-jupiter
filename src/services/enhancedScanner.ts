...
export class EnhancedArbitrageScanner {
  ...
  constructor(connection: Connection, scannerConfig?: Partial<ScannerConfig>) {
    ...
    this.config = {
      ...
      minProfitThreshold:
        scannerConfig?.minProfitThreshold ||
    ...
    this.flashLoanProviders.set(
      "solend",
      new SolendProvider(connection, config.flashLoanProviders.solend, 0.1),
    );
    this.flashLoanProviders.set(
      "mango",
      new MangoProvider(connection, config.flashLoanProviders.mango, 0.15),
    );
    this.flashLoanProviders.set(
      "kamino",
      new KaminoProvider(connection, config.flashLoanProviders.kamino, 0.12),
    );
    this.flashLoanProviders.set(
      "portFinance",
    ...
    this.dexes.set(
      "raydium",
      new RaydiumDEX(connection, config.dexPrograms.raydium),
    );
    ...
    this.dexes.set(
      "mercurial",
      new MercurialDEX(connection, config.dexPrograms.mercurial),
    );
    this.dexes.set(
      "lifinity",
      new LifinityDEX(connection, config.dexPrograms.lifinity),
    );
    this.dexes.set(
      "aldrin",
      new AldrinDEX(connection, config.dexPrograms.aldrin),
    );
    this.dexes.set("crema", new CremaDEX(connection, config.dexPrograms.crema));
    this.dexes.set(
      "meteora",
      new MeteoraDEX(connection, config.dexPrograms.meteora),
    );
    this.dexes.set(
      "phoenix",
      new PhoenixDEX(connection, config.dexPrograms.phoenix),
    );
    this.dexes.set(
      "openbook",
      new OpenBookDEX(connection, config.dexPrograms.openbook),
    );
    this.dexes.set(
      "fluxbeam",
      new FluxBeamDEX(connection, config.dexPrograms.fluxbeam),
    );
  }
  ...
  async startScanning(): Promise<void> {
    ...
    console.log(
      `[Scanner] Polling interval: ${this.config.pollingIntervalMs}ms`,
    );
    console.log(
      `[Scanner] Min profit threshold: ${(this.config.minProfitThreshold * 100).toFixed(2)}%`,
    );
    console.log(
      `[Scanner] Max slippage: ${(this.config.maxSlippage * 100).toFixed(2)}%`,
    );
    console.log(
      `[Scanner] Flash loan providers: ${this.flashLoanProviders.size}`,
    );
    ...
    while (this.isScanning) {
      ...
      const waitTime = Math.max(
        0,
        this.config.pollingIntervalMs - scanDuration,
      );
  ...
  private async performScan(): Promise<void> {
    ...
    if (this.config.enableLiveUpdates && this.scanCount % 10 === 0) {
      console.log(
        `[Scanner] 🔍 Scan #${this.scanCount} | Opportunities found: ${this.opportunitiesFound.length}`,
      );
    }
  ...
  private async scanFlashLoanOpportunities(): Promise<OpportunityAlert[]> {
    ...
    const topTokens = SUPPORTED_TOKENS.filter((t) =>
      ["SOL", "USDC", "USDT", "RAY", "ORCA", "mSOL", "jitoSOL"].includes(
        t.symbol,
      ),
    );
    ...
    for (const tokenA of topTokens) {
      for (const tokenB of topTokens) {
        ...
        for (const [
          providerName,
          provider,
        ] of this.flashLoanProviders.entries()) {
  ...
  private async scanTriangularOpportunities(): Promise<OpportunityAlert[]> {
    ...
    for (const path of commonPaths) {
      const tokens = path
        .map((sym) => SUPPORTED_TOKENS.find((t) => t.symbol === sym))
        .filter((t): t is (typeof SUPPORTED_TOKENS)[0] => t !== undefined);
  ...
  private async scanCrossDexOpportunities(): Promise<OpportunityAlert[]> {
    ...
    for (const [symbolA, symbolB] of tokenPairs) {
      ...
      if (
        !SecurityValidator.validateTokenMint(tokenA.mint) ||
