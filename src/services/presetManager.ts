...
export class PresetManager {
  ...
  async syncToQuickNodeKV(quicknode: {
    kvSet: (key: string, value: unknown, ttl?: number) => Promise<boolean>;
  }): Promise<boolean> {
    try {
      const presetsData = await this.exportPresets();
      const success = await quicknode.kvSet(
        "presets-backup",
        presetsData,
        86400 * 7,
      ); // 7 days TTL
  ...
  async restoreFromQuickNodeKV(quicknode: {
