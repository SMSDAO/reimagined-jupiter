...
export class ProviderManager {
  ...
  private initializeProviders(): void {
    try {
      ...
      console.log(`[ProviderManager] Initialized ${this.providers.size} providers`);
    } catch (error) {
  ...
  async getBestProvider(
    ...
    try {
      ...
      for (const providerName of this.preferredProviders) {
        ...
        if (liquidity >= amount) {
          console.log(`[ProviderManager] Selected provider: ${providerName} (fee: ${provider.getFee()}%)`);
          return { name: providerName, provider };
        }
        
        console.log(`[ProviderManager] ${providerName} has insufficient liquidity (${liquidity} < ${amount})`);
      }
      
      console.warn('[ProviderManager] No provider with sufficient liquidity found');
      return null;
    } catch (error) {
  ...
  setPreferredProviders(providerNames: string[]): void {
    ...
    if (!Array.isArray(providerNames) || providerNames.length === 0) {
      console.error('[ProviderManager] Invalid providerNames: must be non-empty array');
      return;
    }
    ...
    console.log(`[ProviderManager] Updated preferred providers: ${validProviders.join(', ')}`);
  }
  ...
  async getAllProvidersInfo(): Promise<Array<{
    ...
    for (const [name, provider] of this.providers) {
      try {
        ...
        console.error(`[ProviderManager] Error getting info for ${name}:`, error);
        providersInfo.push({
          name,
          fee: provider.getFee(),
          healthy: false,
  ...
  getStatistics(): {
    ...
    const healthyCount = Array.from(this.providerHealth.values()).filter(h => h).length;
