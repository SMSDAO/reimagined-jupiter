...
export class AirdropService {
  ...
  constructor(
    connection: Connection,
    devWallet: string,
    donationPercentage: number = 0.1, // 10% default
  ) {
  ...
  async checkAllEligibility(
    ...
    for (const [protocol, checker] of this.checkers.entries()) {
      try {
        const eligibility = await checker.checkEligibility(
          this.connection,
          walletAddress,
        );
  ...
  async createClaimTransaction(
    ...
    if (options.includeDonation !== false) {
      const donationAmount = this.calculateDonation(
        eligibility.allocationAmount,
      );
  ...
  async trackDonation(
    ...
    console.log(
      `📊 Donation tracked: ${donationAmount} ${donationToken} from ${donorWallet}`,
    );
