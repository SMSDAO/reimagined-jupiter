...
class GXQStudio {
  ...
  constructor() {
    ...
    if (config.solana.walletPrivateKey) {
      try {
        ...
        this.airdropChecker = new AirdropChecker(
          this.connection,
  ...
  private printSystemInfo(): void {
    ...
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log(
      `🌐 QuickNode Integration: ${config.quicknode.rpcUrl ? "✓" : "✗"}`,
    );
    console.log(`💰 Flash Loan Providers: 6`);
    console.log(
      `   - Marginfi (${config.flashLoanProviders.marginfi.toBase58().slice(0, 8)}...) - 0.09% fee`,
    );
    console.log(
      `   - Solend (${config.flashLoanProviders.solend.toBase58().slice(0, 8)}...) - 0.10% fee`,
    );
    console.log(
      `   - Save Finance (${config.flashLoanProviders.saveFinance.toBase58().slice(0, 8)}...) - 0.11% fee`,
    );
    console.log(
      `   - Kamino (${config.flashLoanProviders.kamino.toBase58().slice(0, 8)}...) - 0.12% fee`,
    );
    console.log(
      `   - Mango (${config.flashLoanProviders.mango.toBase58().slice(0, 8)}...) - 0.15% fee`,
    );
    console.log(
      `   - Port Finance (${config.flashLoanProviders.portFinance.toBase58().slice(0, 8)}...) - 0.20% fee`,
    );
    ...
    console.log(
      `📈 Jupiter v6: ${config.jupiter.programId.toBase58().slice(0, 8)}...`,
    );
    ...
    console.log(
      `💎 GXQ Ecosystem: ${config.gxq.tokenMint.toBase58().slice(0, 8)}...`,
    );
  ...
  async checkAirdrops(): Promise<void> {
    ...
    for (const airdrop of airdrops) {
      console.log(
        `  ${airdrop.protocol}: ${airdrop.amount} ${airdrop.tokenMint.slice(0, 8)}...`,
      );
    }
  }
  ...
  async listPresets(): Promise<void> {
    ...
    for (const preset of presets) {
      ...
      console.log(
        `      Max Slippage: ${(preset.maxSlippage * 100).toFixed(2)}%`,
      );
      console.log("");
    }
  }
  ...
  async scanOpportunities(): Promise<void> {
    ...
    for (const preset of presets) {
      ...
      if (preset.strategy === "flash-loan" || preset.strategy === "hybrid") {
        const flashOpps = await this.flashLoanArbitrage.findOpportunities(
          preset.tokens,
        );
        const filtered = flashOpps.filter(
          (o) => o.estimatedProfit >= preset.minProfit,
        );

        if (filtered.length > 0) {
          console.log(
            `  ⚡ Flash Loan: Found ${filtered.length} opportunities`,
          );
          const best = filtered[0];
          console.log(
            `     Best: ${best.provider} - $${best.estimatedProfit.toFixed(4)} profit`,
          );
        }
      }
      ...
      if (preset.strategy === "triangular" || preset.strategy === "hybrid") {
        const triOpps = await this.triangularArbitrage.findOpportunities(
          preset.tokens,
        );
        const filtered = triOpps.filter(
          (o) => o.estimatedProfit >= preset.minProfit,
        );

        if (filtered.length > 0) {
          console.log(
            `  🔺 Triangular: Found ${filtered.length} opportunities`,
          );
          const best = filtered[0];
          console.log(
            `     Best: ${best.path.map((t) => t.symbol).join(" -> ")} - $${best.estimatedProfit.toFixed(4)}`,
          );
        }
      }
    }
  ...
  async showFlashLoanProviders(): Promise<void> {
    ...
    for (const provider of providers) {
      ...
      console.log(
        `    Program ID: ${provider.programId.toBase58().slice(0, 16)}...`,
      );
      console.log("");
    }
  }
  ...
  async analyzeWallet(address?: string): Promise<void> {
    ...
    console.log(
      `Wallet: ${score.address.slice(0, 8)}...${score.address.slice(-8)}`,
    );
    console.log(`\n🏆 Tier: ${score.tier}`);
    ...
    console.log(
      `💰 Estimated Airdrop Value: $${score.estimatedAirdropValue.toLocaleString()}`,
    );
  }
  ...
  async showScannerStats(): Promise<void> {
    ...
    if (stats.recentOpportunities.length > 0) {
      ...
      for (const opp of stats.recentOpportunities) {
        console.log(
          `  ${opp.type} - ${opp.tokens.join(" -> ")} - ${(opp.estimatedProfit * 100).toFixed(3)}%`,
        );
      }
    }
  }
  ...
  async startRealTimeScanner(tokens?: string[]): Promise<void> {
    ...
    this.realTimeScanner.onOpportunityFound((opportunity) => {
      ...
      console.log(
        `  Path: ${opportunity.path.map((t) => t.symbol).join(" -> ")}`,
      );
      console.log(
        `  Estimated Profit: ${opportunity.estimatedProfit.toFixed(6)} ${opportunity.path[0].symbol}`,
      );
      console.log(
        `  Required Capital: ${opportunity.requiredCapital.toFixed(6)} ${opportunity.path[0].symbol}`,
      );
      console.log(
      ...
      console.log(
        `  Est. Slippage: ${(opportunity.estimatedSlippage! * 100).toFixed(2)}%`,
      );
      console.log(
        `  Est. Gas Fee: ${opportunity.estimatedGasFee?.toFixed(9)} SOL`,
      );
    ...
    console.log(
      `  Min Profit: ${(config.minProfitThreshold * 100).toFixed(2)}%`,
    );
  ...
  async scanRealTimeOnce(tokens?: string[]): Promise<void> {
    ...
    const opportunities =
      await this.realTimeScanner.scanForOpportunities(tokenList);
    ...
    for (let i = 0; i < Math.min(opportunities.length, 10); i++) {
      ...
      console.log(
        `   Profit: ${opp.estimatedProfit.toFixed(6)} ${opp.path[0].symbol}`,
      );
      console.log(
        `   Capital Required: ${opp.requiredCapital.toFixed(6)} ${opp.path[0].symbol}`,
      );
...
async function main() {
  ...
  switch (command) {
    ...
    case "db-stats":
      ...
      break;
    case "history":
      await studio.showHistoricalAnalysis(
        args[1] ? parseInt(args[1]) : undefined,
      );
      break;
    case "realtime-scan":
    ...
    default:
      ...
      console.log(
        "  npm start manual      - Manual execution mode (review opportunities)",
      );
      console.log("  npm start providers   - Show flash loan providers");
      console.log("");
      console.log("Enhanced Scanner:");
      console.log(
        "  npm start enhanced-scan [interval]  - Start enhanced scanner (1s default)",
      );
      console.log(
        "  npm start scanner-stats             - Show scanner statistics",
      );
      console.log(
        "  npm start db-stats                  - Show database statistics",
      );
      console.log(
        "  npm start history [days]            - Historical analysis (7 days default)",
      );
      console.log("");
      console.log("Real-Time Arbitrage Scanner (NEW):");
      console.log(
        "  npm start realtime-scan [tokens...] - Start real-time scanner (all tokens default)",
      );
      console.log(
        "  npm start realtime-once [tokens...] - One-time scan for opportunities",
      );
      console.log("  Example: npm start realtime-scan SOL USDC USDT");
      console.log("");
      console.log("Phase 2 Features:");
      console.log(
        "  npm start analyze [address]  - Analyze wallet score and tier",
      );
      console.log("  npm start addresses [action] - Manage address book");
      console.log("  npm start templates          - View route templates");
      console.log(
        "  npm start export [type]      - Export config (presets/templates/addresses)",
      );
      console.log(
        "  npm start sync               - Sync presets to QuickNode KV",
      );
      break;
  }
}
