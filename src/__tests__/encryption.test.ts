...
describe("EncryptionService", () => {
  ...
  describe("encrypt and decrypt", () => {
    ...
    it("should produce different ciphertext for same data (due to random IV)", () => {
      ...
      expect(encryptionService.decrypt(encrypted1, testPassword)).toBe(
        testData,
      );
      expect(encryptionService.decrypt(encrypted2, testPassword)).toBe(
        testData,
      );
    });
  });
  ...
  describe("encryptPrivateKey and decryptPrivateKey", () => {
    ...
    it("should encrypt and decrypt private key", () => {
      ...
      const decrypted = encryptionService.decryptPrivateKey(
        encrypted,
        testPassword,
      );
  ...
  describe("encryptEnvVars and decryptEnvVars", () => {
    ...
    it("should encrypt and decrypt environment variables", () => {
      const encrypted = encryptionService.encryptEnvVars(
        testVars,
        testPassword,
      );
      ...
      const decrypted = encryptionService.decryptEnvVars(
        encrypted,
        testPassword,
      );
