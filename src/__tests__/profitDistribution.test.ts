...
describe("ProfitDistributionService", () => {
  describe("Configuration Validation", () => {
    it("should accept valid configuration with percentages summing to 1.0", () => {
      ...
      const mockDaoWallet = new PublicKey(
      ...
      expect(() => {
        new ProfitDistributionService(connection, {
          ...
          userWalletPercentage: 0.2,
          reserveWalletPercentage: 0.7,
          daoWalletPercentage: 0.1,
  ...
  describe("Profit Calculation", () => {
    it("should correctly calculate 70/20/10 split", () => {
      ...
      expect(Math.floor(profitAmount * 0.7)).toBe(expectedReserve);
      expect(Math.floor(profitAmount * 0.2)).toBe(expectedUser);
      expect(Math.floor(profitAmount * 0.1)).toBe(expectedDao);
    });
  });
});
