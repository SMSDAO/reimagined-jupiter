...
jest.mock('@pythnetwork/client', () => ({
  ...
  getPythProgramKeyForCluster: jest.fn().mockReturnValue(new PublicKey('11111111111111111111111111111111')),
}));
...
describe('PythNetworkIntegration', () => {
  ...
  describe('calculateDynamicSlippage', () => {
    it('should return at least base slippage', async () => {
      ...
      const dynamicSlippage = await pyth.calculateDynamicSlippage('SOL', baseSlippage);
    ...
    it('should return base slippage on error', async () => {
      const baseSlippage = 0.01;
      const dynamicSlippage = await pyth.calculateDynamicSlippage('INVALID_TOKEN', baseSlippage);
