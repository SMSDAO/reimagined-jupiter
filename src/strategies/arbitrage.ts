...
export class FlashLoanArbitrage {
    ...
    for (const providerName of this.providers.keys()) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        for (let j = i + 1; j < tokens.length; j++) {
          ...
          if (
            opportunity &&
            opportunity.estimatedProfit > config.arbitrage.minProfitThreshold
          ) {
  ...
  async executeArbitrage(
    ...
    try {
      console.log(
        `Executing flash loan arbitrage via ${opportunity.provider}...`,
      );
      console.log(
      ...
      const availableLiquidity = await provider.getAvailableLiquidity(
        path[0].mint,
      );
      if (availableLiquidity < loanAmount) {
        console.error(
          `Insufficient liquidity: need ${loanAmount}, available ${availableLiquidity}`,
        );
        return null;
      }
      ...
      const repayAmount =
        loanAmount + Math.floor(loanAmount * (provider.getFee() / 100));
      ...
      console.log(
        `Flash loan arbitrage prepared. Expected profit: ${profit / Math.pow(10, path[0].decimals)}`,
      );
      ...
      console.warn(
        "Transaction execution requires proper instruction building and wallet signing",
      );
      console.log(
        `Transaction would contain ${flashLoanInstructions.length} instructions`,
      );
      return null;
    } catch (error) {
...
export class TriangularArbitrage {
      ...
      for (let j = 0; j < tokens.length; j++) {
        for (let k = 0; k < tokens.length; k++) {
          if (i !== j && j !== k && i !== k) {
            ...
            const opportunity = await this.checkTriangularPath(
              tokenA,
              tokenB,
              tokenC,
            );
  ...
  async executeArbitrage(
    ...
    try {
      ...
      let currentAmount = Math.floor(
        opportunity.requiredCapital * Math.pow(10, path[0].decimals),
      );
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        console.log(
          `Swap ${i + 1}: ${fromToken.symbol} -> ${toToken.symbol}, amount: ${currentAmount}`,
        );
      ...
      console.log(
        `Triangular arbitrage executed successfully. Final profit: ${profit}`,
      );
      ...
      console.warn(
        "Transaction execution requires wallet signing and proper bundling",
      );
      return null;
    } catch (error) {
