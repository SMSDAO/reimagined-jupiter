...
export class ExecutionLogger {
  ...
  private printExecutionSummary(log: ExecutionLog): void {
    ...
    if (log.actualProfit !== undefined) {
      ...
      console.log(
        `  Difference: $${difference.toFixed(6)} (${percentage.toFixed(2)}%)`,
      );
    }
    ...
    if (log.fees) {
      ...
      console.log(
        `  Prioritization: ${log.fees.prioritization.toFixed(6)} SOL`,
      );
      const totalFees =
        log.fees.transaction +
        (log.fees.flashLoan || 0) +
        log.fees.prioritization;
    ...
    if (log.profitDistribution) {
      console.log("\n💸 Profit Distribution:");
      console.log(
        `  Reserve (70%): $${log.profitDistribution.reserve.toFixed(6)}`,
      );
      console.log(
        `  Gas/Slippage (20%): $${log.profitDistribution.gasSlippage.toFixed(6)}`,
      );
  ...
  async getStatistics(since?: Date): Promise<{
    ...
    try {
      ...
      if (since) {
        logs = logs.filter(
          (log: ExecutionLog) => new Date(log.timestamp) >= since,
        );
      }

      const successful = logs.filter(
        (log: ExecutionLog) => log.status === "success",
      );
      const failed = logs.filter(
        (log: ExecutionLog) =>
          log.status === "failed" || log.status === "simulation_failed",
      );

      const totalProfit = successful.reduce(
        (sum: number, log: ExecutionLog) => sum + (log.actualProfit || 0),
        0,
      );
      const avgProfit =
