...
export class TransactionExecutor {
  ...
  async calculateDynamicPriorityFee(
    ...
    try {
      ...
      console.log(
        `💎 Dynamic Priority Fee: ${microLamports} microLamports (${urgency} urgency)`,
      );
    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
        try {
          ...
          const currentHeight =
            await this.connection.getBlockHeight(commitment);
  ...
  async buildVersionedTransactionWithComputeBudget(
    ...
    try {
      ...
      console.log(
        `   Compute limit: ${feeConfig.computeUnitLimit.toLocaleString()} units`,
      );
  ...
  async executeTransactionWithSimulation(
    ...
    if (!simulationSuccess) {
      return {
        success: false,
        error:
          "Transaction simulation failed - transaction would fail on-chain",
      };
    }
  ...
  async executeVersionedTransactionWithSimulation(
    ...
    const simulationSuccess =
      await this.simulateVersionedTransaction(transaction);
    if (!simulationSuccess) {
      return {
        success: false,
        error:
    ...
    // Execute the transaction
    return await this.executeVersionedTransaction(
      transaction,
      signers,
      commitment,
    );
  ...
  async executeAtomicBundleWithJito(
    ...
    if (!this.jitoIntegration) {
      console.warn(
        "⚠️  Jito integration not enabled, falling back to regular transaction execution",
      );
    ...
    try {
      ...
      const result: JitoBundleResult =
        await this.jitoIntegration.executeAtomicBundle(
          transactions,
          signers,
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      console.error(
        "❌ Error executing atomic bundle with Jito:",
        errorMessage,
      );
      return {
        success: false,
