...
export function calculateFarcasterScore(user: FarcasterUser | null): number {
  ...
  const hasVerifiedAddresses =
    (user.verified_addresses?.eth_addresses?.length ?? 0) > 0 ||
  ...
  const isInfluencer =
    followers > 1000 && following > 0 && following / followers < 0.5;
  if (isInfluencer) {
    score += 10;
  }
...
export async function getGMCasts(fid: number, daysBack = 30): Promise<GMStats> {
  try {
    ...
    const totalLikes = gmCasts.reduce(
      (sum, cast) => sum + (cast.reactions?.likes_count || 0),
      0,
    );
    const totalRecasts = gmCasts.reduce(
      (sum, cast) => sum + (cast.reactions?.recasts_count || 0),
      0,
    );
    const totalReplies = gmCasts.reduce(
    ...
    const engagementRate =
      gmCasts.length > 0 ? (totalEngagement / gmCasts.length).toFixed(2) : 0;
...
export function calculateGMScore(gmStats: GMStats): number {
  ...
  const consistencyPercentage =
    (gmStats.gm_consistency_days / gmStats.period_days) * 100;
...
export function calculateTrustScore(
  ...
  const farcasterComponent = farcasterScore * 0.3;

  // 3. GM Score (20% weight)
  const gmComponent = gmScore * 0.2;

  // 4. Age Bonus (10% weight)
  // Logarithmic scale: 30 days = 3pts, 90 days = 6pts, 365 days = 10pts
  const ageDays = walletAgeDays || 0;
  const ageBonus = ageDays > 0 ? Math.min(10, Math.log10(ageDays + 1) * 4) : 0;

  // Calculate total trust score
  const trustScore = Math.round(
    inverseRisk + farcasterComponent + gmComponent + ageBonus,
  );
