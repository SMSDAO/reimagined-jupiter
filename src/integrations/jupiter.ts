...
export class JupiterV6Integration {
  ...
  async getQuote(
    ...
    if (
      ...
      console.error(
        "[Jupiter] Invalid parameters: inputMint and outputMint are required and must not be empty",
      );
      return null;
    }
    ...
    if (!amount || amount <= 0) {
      console.error(
        "[Jupiter] Invalid amount: must be greater than 0, received:",
        amount,
      );
      return null;
    }
    ...
    try {
      console.log(
        `[Jupiter] Fetching quote: ${inputMint.slice(0, 8)}... -> ${outputMint.slice(0, 8)}..., amount: ${amount}`,
      );
      ...
      console.log(
        `[Jupiter] Quote received: out amount ${response.data.outAmount}`,
      );
  ...
  async getSwapTransaction(
    ...
    try {
      console.log(
        `[Jupiter] Creating swap transaction for user: ${userPublicKey.slice(0, 8)}...`,
      );
      ...
      if (!response.data || !response.data.swapTransaction) {
        console.error(
          "[Jupiter] Invalid swap response: missing swapTransaction",
        );
        return null;
      }
  ...
  async executeSwap(
    ...
    try {
      console.log(
        `[Jupiter] Executing swap for ${amount} of ${inputMint.slice(0, 8)}...`,
      );
      ...
      if (!swapTransaction) {
        console.error(
          "[Jupiter] Failed to get swap transaction, cannot execute swap",
        );
        return null;
      }
      ...
      console.warn(
        "[Jupiter] Transaction execution requires wallet signing - transaction object ready but not submitted",
      );
  ...
  async getPriceInUSD(tokenMint: string): Promise<number | null> {
    ...
    if (!tokenMint || tokenMint.trim() === "") {
      console.error(
        "[Jupiter] Invalid tokenMint: token mint address is required and must not be empty",
      );
      return null;
    }
    ...
    try {
      console.log(
        `[Jupiter] Fetching USD price for token: ${tokenMint.slice(0, 8)}...`,
      );

      // Use v6 price API with fallback to v4
      const priceApiUrl =
        config.jupiter.priceApiUrl || "https://price.jup.ag/v6";
      ...
      if (price === undefined || price === null) {
        console.warn(
          `[Jupiter] No price available for token: ${tokenMint.slice(0, 8)}...`,
        );
        return null;
      }
