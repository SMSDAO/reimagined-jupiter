...
export class QuickNodeIntegration {
  ...
  constructor() {
    this.rpcConnection = new Connection(
      config.quicknode.rpcUrl || config.solana.rpcUrl,
    );
  ...
  async invokeFunction(
    ...
    if (!functionName) {
      console.error(
        "[QuickNode] Invalid functionName: function name is required",
      );
      return null;
    }
    ...
    try {
      ...
      console.log(
        `[QuickNode] Function ${functionName} completed successfully`,
      );
  ...
  async getTokenPrice(tokenMint: string): Promise<number | null> {
    try {
      ...
      const result = (await this.invokeFunction("getTokenPrice", {
        tokenMint,
      })) as { price?: number };
  ...
  async cacheArbitrageOpportunity(
