...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    console.log(
      `📊 Metrics request from admin: ${verification.payload.username}`,
    );
    ...
    const tradesLast24h = metricsData.trades.filter(
      (t) => t.timestamp >= oneDayAgo,
    );
    const tradesLastWeek = metricsData.trades.filter(
      (t) => t.timestamp >= oneWeekAgo,
    );
    const tradesLastMonth = metricsData.trades.filter(
      (t) => t.timestamp >= oneMonthAgo,
    );
    ...
    const successRate =
      tradesCount > 0 ? (successCount / tradesCount) * 100 : 0;
    ...
    const avgExecutionTime =
      metricsData.trades.length > 0
        ? metricsData.trades.reduce((sum, t) => sum + t.executionTime, 0) /
          metricsData.trades.length
        : 0;
