...
async function testBulkPrices() {
  ...
  const prices = await getBulkTokenPrices([SOL_MINT, USDC_MINT, BONK_MINT, JUP_MINT]);
  
  console.log('Bulk prices:');
  Object.entries(prices).forEach(([mint, price]) => {
    const symbol = mint === SOL_MINT ? 'SOL' :
                   mint === USDC_MINT ? 'USDC' :
                   mint === BONK_MINT ? 'BONK' :
                   mint === JUP_MINT ? 'JUP' : 'UNKNOWN';
...
async function testPortfolioValue() {
  ...
  portfolio.tokens.forEach(token => {
    const symbol = token.mint === SOL_MINT ? 'SOL' :
                   token.mint === USDC_MINT ? 'USDC' :
                   token.mint === BONK_MINT ? 'BONK' : 'UNKNOWN';
    console.log(`  ${symbol}: ${token.amount} @ $${token.price.toFixed(6)} = $${token.value.toFixed(2)}`);
  });
