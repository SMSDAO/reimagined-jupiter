...
export class VercelConfigManager {
  ...
  logConfiguration(): void {
    ...
    if (this.config.isVercel) {
      ...
      console.log(
        `Git Commit: ${this.config.gitCommitSha?.substring(0, 7) || "N/A"}`,
      );

      const optimizations = this.getOptimizations();
      console.log(
        `Edge Runtime: ${optimizations.edgeRuntime ? "Enabled" : "Disabled"}`,
      );
      console.log(
        `Revalidation: ${optimizations.revalidate !== false ? `${optimizations.revalidate}s` : "Disabled"}`,
      );
    }
...
export class EdgeRuntimeChecker {
  ...
  static isFeatureAvailable(
    feature: "crypto" | "fs" | "buffer" | "stream",
  ): boolean {
  ...
  static checkReadiness(): {
    ...
    if (EnvironmentDetector.isProduction()) {
      if (
        ...
        warnings.push(
          "Consider using premium RPC endpoints (Helius or QuickNode) for production",
        );
      }
    }
