...
export class JupiterAPIClient {
  ...
  async getTokenPrice(tokenMint: string): Promise<number | null> {
    try {
      ...
      const response = await fetch(
        `${this.priceApiUrl}/price?ids=${tokenMint}`,
        {
          signal: AbortSignal.timeout(this.timeout),
        },
      );
  ...
  async getTokenPrices(
    ...
    try {
      ...
      console.log(
        `[Jupiter] Fetching USD prices for ${tokenMints.length} tokens`,
      );
      ...
      const data = await response.json();
      console.log(
        `[Jupiter] Fetched prices for ${Object.keys(data.data || {}).length} tokens`,
      );
      return data.data || {};
    } catch (error) {
  ...
  async getTokenList(): Promise<JupiterToken[]> {
    try {
      ...
      if (!response.ok) {
        console.error(
          `[Jupiter] Token list API error: ${response.status} ${response.statusText}`,
        );
        return [];
      }
...
export const jupiterApi = {
  ...
  getSwapTransaction: (params: JupiterSwapParams) =>
    getJupiterClient().getSwapTransaction(params),
  getTokenPrice: (tokenMint: string) =>
    getJupiterClient().getTokenPrice(tokenMint),
  getTokenPrices: (tokenMints: string[]) =>
    getJupiterClient().getTokenPrices(tokenMints),
