...
export class ArbitrageScanner {
  ...
  private async updatePrices(): Promise<void> {
    try {
      ...
      const response = await fetch(`${API.jupiterPrice()}/price?ids=${popularTokens.join(',')}`);
      ...
      for (const pair of tokenPairs) {
        ...
        for (const provider of flashProviders) {
          ...
          if (netProfit > minProfitPercent / 100) {
            ...
            const liquidityScore = Math.min(100, jupiterQuote.outAmount / 100000000); // Normalize
  ...
  private async getJupiterQuote(inputMint: string, outputMint: string): Promise<any> {
    try {
  ...
  private async scanTriangularArbitrage(minProfitPercent: number): Promise<void> {
    ...
    for (const route of triangularRoutes) {
      try {
        ...
        if (netProfit > minProfitPercent / 100) {
          ...
          const confidence = Math.max(60, Math.min(85, 80 - (avgPriceImpact * 50)));
        ...
        console.error('[ArbitrageScanner] Error in triangular arbitrage scan:', error);
      }
    }
  }
  ...
  private getRouteFromQuote(quote: any): string {
    ...
    const dexNames = quote.routePlan.map((r: any) => r.swapInfo?.label || 'Unknown');
