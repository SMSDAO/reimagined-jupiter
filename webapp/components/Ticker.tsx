      ...
      <div className="flex items-center space-x-2 mb-2">
        ...
        {status.connected && (
          <span className="text-xs text-gray-500">
            ({Object.values(status.providerStatus).filter(Boolean).length}{" "}
            providers active)
          </span>
        )}
      </div>
        ...
        {data.prices.map((price) => (
          ...
          >
            <div className="flex items-center space-x-2">
              <span
                className={`font-semibold ${compact ? "text-sm" : "text-base"} text-white`}
              >
                {price.symbol}
              </span>
              <span
                className={`${getPriceChangeColor(price.symbol)} ${compact ? "text-xs" : "text-sm"}`}
              >
                {getPriceChangeIndicator(price.symbol)}
              </span>
            </div>
            <div
              className={`${compact ? "text-sm" : "text-lg"} font-mono font-bold text-blue-400`}
            >
            ...
            <div className="text-xs text-gray-600 mt-1">{price.source}</div>
          </div>
        ))}
      </div>
