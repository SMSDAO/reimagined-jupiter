...
export default function PendingApprovals() {
  ...
  const handleApprove = async (approvalId: string) => {
    ...
    try {
      ...
      alert(
        `❌ Failed to approve transaction: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
  ...
  const handleReject = async (approvalId: string) => {
    ...
    try {
      ...
      alert(
        `❌ Failed to reject transaction: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
      setProcessingId(null);
                ...
                <div className="text-right">
                  <div
                    className={`text-2xl font-bold ${
                      approval.valueAtRisk > 5
                        ? "text-red-400"
                        : approval.valueAtRisk > 1
                          ? "text-yellow-400"
                          : "text-green-400"
                    }`}
                  >
