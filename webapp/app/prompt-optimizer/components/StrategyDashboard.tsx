...
export default function StrategyDashboard() {
  const [selectedStrategy, setSelectedStrategy] =
    useState<ArbitrageStrategy | null>(null);
          ...
          {defaultStrategies.map((strategy) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-semibold text-white ${getRiskBgColor(strategy.riskLevel)}`}
                >
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-semibold">
                    {(
                      selectedStrategy.parameters.minProfitThreshold * 100
                    ).toFixed(2)}
                    %
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-semibold">
                    {(selectedStrategy.parameters.maxSlippage * 100).toFixed(2)}
                    %
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-semibold">
                    {selectedStrategy.parameters.flashLoanProvider || "N/A"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-semibold ${getRiskColor(selectedStrategy.riskLevel)}`}
                  >
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-semibold">
                    {strategyScore.toFixed(0)}/100
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Status:</span>
                  <span
                    className={`font-semibold ${selectedStrategy.enabled ? "text-green-400" : "text-red-400"}`}
                  >
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedStrategy.parameters.tokens.map((token) => (
                <span
                  key={token}
                  className="px-3 py-1 bg-purple-600 text-white rounded-lg text-sm font-semibold"
                >
                  {token}
                </span>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPriceDifference(Number(e.target.value) / 100)
                  }
                  className="w-full bg-gray-800 text-white px-4 py-2 rounded-lg border border-purple-500/30 focus:border-purple-500 focus:outline-none"
                />
              </div>
