...
export function useStrategies() {
  const [strategies, setStrategies] =
    useState<ArbitrageStrategy[]>(defaultStrategies);
  const [selectedStrategy, setSelectedStrategy] =
    useState<ArbitrageStrategy | null>(null);
  const [loading] = useState(false);

  const getStrategy = useCallback(
    (id: string): ArbitrageStrategy | undefined => {
  ...
  const updateStrategy = useCallback(
    (id: string, updates: Partial<ArbitrageStrategy>) => {
      setStrategies((prev) =>
  ...
  const calculateProfit = useCallback(
    (strategyId: string, amount: number, priceDifference: number): number => {
  ...
  const optimizeParameters = useCallback(
    (strategyId: string, marketVolatility: "low" | "medium" | "high") => {
      const strategy = getStrategy(strategyId);
      if (!strategy) return;

      const optimizedParams = optimizeStrategyParameters(
        strategy,
        marketVolatility,
      );
