...
export const defaultStrategies: ArbitrageStrategy[] = [
  {
    ...
    prompt:
      "Analyze stablecoin arbitrage opportunities between {{dex1}} and {{dex2}} for {{tokenPair}}. Calculate optimal flash loan amount from {{provider}} considering {{fee}}% fee. Evaluate profit after slippage of {{slippage}}%.",
    parameters: {
  ...
  },
  {
    ...
    prompt:
      "Find triangular arbitrage cycles starting with SOL through {{intermediateTokens}}. Analyze paths: SOL → {{token1}} → {{token2}} → SOL. Calculate expected profit considering {{swapFees}}% total fees and {{networkFee}} SOL network cost.",
    parameters: {
  ...
  },
  {
    ...
    prompt:
      "Evaluate arbitrage between {{lstToken}} and SOL across {{dexList}}. Consider {{depegPercentage}}% depeg opportunity. Factor in unstaking period of {{unstakeDays}} days and current APY of {{apy}}%.",
    parameters: {
  ...
  },
  {
    ...
    prompt:
      "Identify volatile memecoin {{token}} arbitrage between {{dex1}} and {{dex2}}. Price difference: {{priceDiff}}%. Analyze liquidity depth of {{liquidityAmount}} and volatility of {{volatilityPercent}}%. Calculate MEV protection requirements.",
    parameters: {
  ...
  },
  {
    ...
    prompt:
      "Scan for arbitrage across all major Solana DEXs: {{dexList}}. For token {{token}}, compare prices and identify best buy/sell venues. Calculate profit after {{totalFees}}% fees. Recommend flash loan provider if beneficial.",
    parameters: {
      minProfitThreshold: 0.005, // 0.5%
      maxSlippage: 0.015, // 1.5%
...
export function calculateExpectedProfit(
  ...
  const flashLoanFee = strategy.parameters.flashLoanProvider
    ? getFlashLoanFee(strategy.parameters.flashLoanProvider)
    : 0;
...
export function scoreStrategy(
  ...
  if (strategy.riskLevel === "high" && currentMarketData.volatility > 10)
    score += 15;
  if (strategy.riskLevel === "low" && currentMarketData.volatility < 3)
    score += 10;
 ...
 */
export function recommendStrategy(marketConditions: {
  ...
  const strategiesWithTrending = candidates.filter((s) =>
    s.parameters.tokens.some((t) =>
      marketConditions.trendingTokens.includes(t),
    ),
  );
 ...
 */
export function getStrategiesByRisk(
