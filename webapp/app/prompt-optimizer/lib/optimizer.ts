...
function calculateSpecificity(prompt: string): number {
  ...
  if (
    ...
    prompt.toLowerCase().includes("maximum")
  )
    score += 10;
...
export function getSuggestions(prompt: string): string[] {
  ...
  if (metrics.clarity < 70) {
    suggestions.push(
      "Add clear structure with numbered steps or bullet points",
    );
    suggestions.push(
      "Include specific action verbs (analyze, calculate, evaluate)",
    );
  }
  ...
  if (metrics.specificity < 70) {
    ...
    suggestions.push(
      "Include domain-specific terminology (e.g., DEX names, token symbols)",
    );
...
export function validatePrompt(prompt: string): {
  ...
  if (
    !prompt.match(
      /\b(analyze|calculate|evaluate|generate|provide|create|explain)\b/i,
    )
  ) {
    warnings.push(
      "No clear action verb found - consider adding explicit instructions",
    );
  }
