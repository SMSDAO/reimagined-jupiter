...
export default function SniperPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handlePoolCreation as EventListener,
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "wallet-connected",
      handleWalletConnect as EventListener,
    );
  ...
  const startMonitoring = () => {
    ...
    console.log(
      "[SniperUI] Monitoring active - waiting for pool creation events...",
    );
  ...
  const snipeToken = async (target: SniperTarget) => {
    ...
    try {
      ...
      if (isNaN(amount) || amount <= 0) {
        alert(
          "Invalid buy amount. Please enter a valid number greater than 0.",
        );
        return;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Failed to prepare snipe transaction",
        );
      }
      ...
      alert(
        `Failed to snipe ${target.name}. Error: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4">
      ...
      >
        ...
        <p className="text-sm sm:text-base text-gray-300 mb-6 sm:mb-8">
          Monitor and snipe new token launches across Pump.fun + Raydium, Orca,
          Meteora, Phoenix & more DEXs
        </p>
            ...
            {targets.length > 0 ? (
                        ...
                        <div className="flex-1 min-w-0 w-full">
                          ...
                          <div className="flex flex-wrap gap-3 sm:gap-4">
                            <span className="text-green-400 text-xs sm:text-sm">
                              💰 ${target.liquidity.toLocaleString()}
                            </span>
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/10 text-center">
                ...
                <p className="text-gray-400 text-sm">
                  Watching {platforms.filter((p) => p.active).length} platforms
                </p>
              </div>
            ) : (
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/10 text-center">
