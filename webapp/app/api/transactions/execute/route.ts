...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              "Legacy transactions must be signed client-side for security. Use versioned transactions for server-side execution, or sign the transaction before sending.",
          },
          { status: 400 },
        );
      }
      ...
      if (result.success) {
        console.log(
          `✅ Transaction executed successfully: ${result.signature}`,
        );
        return NextResponse.json({
          success: true,
...
export async function GET() {
  return NextResponse.json({
    ...
    method: "POST",
    description:
      "Execute Solana transactions with resilient connection handling",
    parameters: {
      transaction: "Base64 encoded transaction (required)",
      isVersioned: "boolean - Is this a versioned transaction? (required)",
      urgency:
        "'low' | 'medium' | 'high' | 'critical' (optional, default: 'medium')",
      commitment: "Commitment level (optional, default: 'confirmed')",
    },
    features: [
