...
export default function ArbitragePage() {
  ...
  const flashProviders = [
    ...
    { name: "Solend", fee: 0.1, liquidity: "$180M" },
  ...
  const startScanning = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/arbitrage/scan", {
        ...
        body: JSON.stringify({
          minProfit: 0.5, // Use default, can be made configurable
  ...
  const stopScanning = () => {
    setScanning(false);
    console.log("[Arbitrage] Stopped scanning");
  };
  ...
  const executeArbitrage = async (opp: ArbitrageOpportunity) => {
    ...
    try {
      ...
      alert(
        `❌ Error executing arbitrage: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };
        ...
        if (response.ok) {
          ...
          if (data.opportunities && Array.isArray(data.opportunities)) {
            ...
            const filteredOpportunities = data.opportunities.filter(
              (opp: ArbitrageOpportunity) => opp.profitPercent >= 0.5,
            );
            setOpportunities(filteredOpportunities);

            // Auto-execute if enabled
            if (settings.autoExecute && filteredOpportunities.length > 0) {
              const topOpp = filteredOpportunities[0];
              console.log(
                "[Arbitrage] Auto-executing top opportunity:",
                topOpp,
              );
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 sm:mb-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <InstructionPanel pageType="arbitrage" />
          </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-xs sm:text-sm">
                          {opp.provider && (
                            <span className="text-gray-300">
                              Provider:{" "}
                              <span className="text-white">{opp.provider}</span>
                            </span>
                          )}
                          <span className="text-gray-300">
                            Route:{" "}
                            <span className="text-white">{opp.route}</span>
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                        ...
                        <button
                          ...
                          className="bg-green-600 hover:bg-green-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-bold text-sm sm:text-base shadow-lg whitespace-nowrap disabled:opacity-50"
                        >
            ...
            {opportunities.length === 0 && scanning && (
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/10 text-center">
                ...
                <p className="text-gray-400 text-sm">
                  Monitoring 5 flash loan providers and 8+ DEXs
                </p>
              </div>
            )}
          </div>
