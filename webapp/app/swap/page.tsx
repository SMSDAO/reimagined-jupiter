...
export default function SwapPage() {
  ...
  const tokens =
    tokenList.length > 0 ? tokenList.map((t) => t.symbol) : defaultTokens;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.tokens) {
            ...
            console.log(
              "[Swap] Loaded Jupiter token list:",
  ...
  const getQuote = async () => {
    ...
    try {
      const jupiterApiUrl =
        process.env.NEXT_PUBLIC_JUPITER_API_URL || "https://quote-api.jup.ag";
  ...
  const getPriorityFeeLamports = (level: string): number => {
    ...
    return (
      PRIORITY_FEES[level as keyof typeof PRIORITY_FEES] || PRIORITY_FEES.medium
    );
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 sm:mb-8">
          ...
          <div className="lg:col-span-2">
              ...
              {priceImpact !== null && (
                <div className="mb-4 p-3 bg-yellow-900/30 border border-yellow-500/50 rounded-lg">
                  <div className="flex justify-between text-sm">
                    ...
                    <span
                      className={`font-bold ${priceImpact > 1 ? "text-red-400" : "text-green-400"}`}
                    >
            ...
            <div className="mt-4 text-center text-sm text-gray-400">
              ...
              <span className="text-purple-400">monads.solana</span>
            </div>
          </div>
        </div>
      </motion.div>
